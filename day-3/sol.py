...
def maybe_get_gear_ratio_beside_part(points_to_numbers, x, y, rows, cols):
  unique_nums = set()
  if x-1 >= 0 and x-1 < rows:
    if y-1 >= 0 and y-1 < cols and (x-1, y-1) in points_to_numbers: unique_nums.add(points_to_numbers[(x-1, y-1)])
    if y >= 0 and y < cols and (x-1, y) in points_to_numbers: unique_nums.add(points_to_numbers[(x-1, y)])
    if y+1 >= 0 and y+1 < cols and (x-1, y+1) in points_to_numbers: unique_nums.add(points_to_numbers[(x-1, y+1)])

  if y-1 >= 0 and y-1 < cols and (x, y-1) in points_to_numbers: unique_nums.add(points_to_numbers[(x, y-1)])
  if y+1 >= 0 and y+1 < cols and (x, y+1)  in points_to_numbers: unique_nums.add(points_to_numbers[(x, y+1)])
...
def solution2(filename):
  ...
  for x in range(rows):
    for y in range(cols):
      point = schematic[x][y]
      if point == '*':
        ans += maybe_get_gear_ratio_beside_part(points_to_numbers, x, y, rows, cols)
