...
def get_hand_type(hand):
    ...
    num_jokers = counts["J"]
    if num_jokers > 0:
        freqs = calculate_best_hand(freqs, num_jokers)
...
def hand_comparator(hand_one, hand_two):
    ...
    for card_one, card_two in zip(hand_one, hand_two):
        if CARD_STRENGTHS[card_one] > CARD_STRENGTHS[card_two]:
            return 1
        elif CARD_STRENGTHS[card_one] < CARD_STRENGTHS[card_two]:
            return -1
