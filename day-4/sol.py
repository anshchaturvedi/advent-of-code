...
def process_cards(cards, part_two=False):
  ...
  for idx, card in enumerate(cards):
    ...
    for num in first_part.split(' '): 
      if num != '': key_set.append(int(num))
    for num in second_part.split(' '): 
      if num != '': value_set.append(int(num))

    if part_two: processed_cards[idx+1] = (tuple(key_set), value_set)
    else: processed_cards[tuple(key_set)] = value_set
...
def process_cards_and_get_score2(cards):
  ...
  for card_number, cards in processed_cards.items():
    ...
    for card in game_cards:
      if card in my_cards: matching_cards += 1
  ...
  ans = 0
  for val in card_counts.values(): ans += val
  return ans
